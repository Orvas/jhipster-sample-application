...
public class FreeSpanResourceIT {
    ...
    @Test
    @Transactional
    public void getAllFreeSpansByIdIsEqualToSomething() throws Exception {
        // Get already existing entity
        BaseClass id = freeSpan.getId();
        freeSpanRepository.saveAndFlush(freeSpan);
        Long idId = id.getId();

        // Get all the freeSpanList where id equals to idId
        defaultFreeSpanShouldBeFound("idId.equals=" + idId);

        // Get all the freeSpanList where id equals to idId + 1
        defaultFreeSpanShouldNotBeFound("idId.equals=" + (idId + 1));
    }
    ...
    public void getAllFreeSpansByFreeSpanHistIsEqualToSomething() throws Exception {
        ...
        em.flush();
        freeSpan.addFreeSpanHist(freeSpanHist);
        freeSpanRepository.saveAndFlush(freeSpan);
