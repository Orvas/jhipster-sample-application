...
export interface IPipelineProps extends StateProps, DispatchProps, RouteComponentProps<{ url: string }> {}

export type IPipelineState = IPaginationBaseState;

export class Pipeline extends React.Component<IPipelineProps, IPipelineState> {
  ...
  render() {
    const { pipelineList, match, totalItems } = this.props;
    return (
      <div>
          ...
          <Table responsive>
            ...
            <tbody>
              {pipelineList.map((pipeline, i) => (
                <tr key={`entity-${i}`}>
                  <td>
                    <Button tag={Link} to={`${match.url}/${pipeline.id}`} color="link" size="sm">
                      {pipeline.id}
                    </Button>
                  </td>
                  <td>
                    <TextFormat type="date" value={pipeline.dateCreate} format={APP_DATE_FORMAT} />
                  </td>
                  <td>
                    <TextFormat type="date" value={pipeline.dateEdit} format={APP_DATE_FORMAT} />
                  </td>
                  ...
                  <td className="text-right">
                    <div className="btn-group flex-btn-group-container">
                      ...
                      <Button tag={Link} to={`${match.url}/${pipeline.id}/edit`} color="primary" size="sm">
                        <FontAwesomeIcon icon="pencil-alt" /> <span className="d-none d-md-inline">Edit</span>
                      </Button>
                      <Button tag={Link} to={`${match.url}/${pipeline.id}/delete`} color="danger" size="sm">
...
const mapStateToProps = ({ pipeline }: IRootState) => ({
  pipelineList: pipeline.entities,
