...
export type IPipeState = IPaginationBaseState;

export class Pipe extends React.Component<IPipeProps, IPipeState> {
  state: IPipeState = {
  ...
  render() {
    const { pipeList, match, totalItems } = this.props;
    return (
      <div>
        <h2 id="pipe-heading">
          Pipes
            ...
            <tbody>
              {pipeList.map((pipe, i) => (
                <tr key={`entity-${i}`}>
                  ...
                  <td>
                    <TextFormat type="date" value={pipe.dateCreate} format={APP_DATE_FORMAT} />
                  </td>
                  <td>
                    <TextFormat type="date" value={pipe.dateEdit} format={APP_DATE_FORMAT} />
                  </td>
                  <td>{pipe.creator}</td>
                  <td>{pipe.editor}</td>
                  ...
                  <td className="text-right">
                    <div className="btn-group flex-btn-group-container">
                      ...
                      <Button tag={Link} to={`${match.url}/${pipe.id}/edit`} color="primary" size="sm">
                        <FontAwesomeIcon icon="pencil-alt" /> <span className="d-none d-md-inline">Edit</span>
                      </Button>
                      <Button tag={Link} to={`${match.url}/${pipe.id}/delete`} color="danger" size="sm">
