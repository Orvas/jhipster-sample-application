...
import { IBaseClass, defaultValue } from 'app/shared/model/base-class.model';
...
export const getEntities: ICrudGetAllAction<IBaseClass> = (page, size, sort) => {
  ...
  return {
    ...
    payload: axios.get<IBaseClass>(`${requestUrl}${sort ? '&' : '?'}cacheBuster=${new Date().getTime()}`)
  };
};
